...
class Ship(Sprite, GameConfigure):
    ...
    def __init__(self, window):
        ...
        self.decisionPointRect.centerx = self.imageRect.centerx
        self.decisionPointRect.centery = self.imageRect.centery
        self.centerx = float(self.decisionPointRect.centerx)
        self.centery = float(self.decisionPointRect.centery)
        # move flag
    ...
    def update(self):
        ...
        if self.aliveFlag == True:
            ...
            if self.movingDown and self.imageRect.bottom < self.windowRect.bottom:
                self.centery += speed
                pass
            self.decisionPointRect.centerx = int(self.centerx)
            self.decisionPointRect.centery = int(self.centery)
            ...
            # not alive
            self.boomEffect.setCenter(self.decisionPointRect.centerx, self.decisionPointRect.centery)
    ...
    def blitme(self):
        ...
        if self.aliveFlag:            
            ...
            if self.speed == self.shipSlowSpeed:
                self.window.blit(self.decisionPoint, self.decisionPointRect)
                pass
            # show batterys
...
class ShipBattery(Sprite, GameConfigure):
    ...
    def __init__(self, ship, radius=64, initialPhase=0):
        ...
        self.circleCenterx = self.ship.decisionPointRect.centerx
        self.circleCentery = self.ship.decisionPointRect.centery
        self.angularVelocity = 3.1415926/180
        self.rect.centery = self.ship.decisionPointRect.centery - self.radius * math.cos(initialPhase)
        self.rect.centerx = self.ship.decisionPointRect.centerx + self.radius * math.sin(initialPhase)
        self.bullets = Group()
